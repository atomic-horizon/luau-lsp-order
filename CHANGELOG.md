...
- Recover from access-violation crashes in the old type solver on Windows. `Frontend::check` calls in `checkSimple`/`checkStrict` are now SEH-guarded; crashes are reported to Sentry as handled exceptions, the affected module is marked dirty for retry, and the LSP keeps running instead of terminating the request thread.
- Reverted the callable-table typing of `shared` introduced in 1.67.2 (`MetatableType` with `[any]: any` indexer and `__call`); that shape triggered crashes during dependency-cascade unification. `shared` is once again a plain function `(string, boolean?) -> any` with the magic resolver attached. Field access on `shared` (e.g. `shared.foo = 5`) goes back to typing as `any`.
